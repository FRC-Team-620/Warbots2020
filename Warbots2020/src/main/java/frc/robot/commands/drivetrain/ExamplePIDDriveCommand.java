...
public class ExamplePIDDriveCommand extends CommandBase {
    ...
    @Override
    public void execute() {
        this.dt.curvatureInput(this.pid.calculate(this.dt.getDistance()), 0, false); //Calculate and use next pid Value based off of current distance.
    }

    @Override
    public boolean isFinished() {
        return this.pid.atSetpoint(); //Finish command when we reach setpoint. (On real bot you may also want to include a timeout)
    }
